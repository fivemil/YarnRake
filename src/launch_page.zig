...
pub fn html() []const u8 {
    return
        ...
        \\input[type=text],select{width:100%;box-sizing:border-box;background:#0f1117;border:1px solid #3a4150;color:#eee;border-radius:8px;padding:8px 10px}
        ...
        \\a{color:#5e9bdc} .ok{color:#6dcea4} .warn{color:#e6b35a}
        \\<\/style><\/head><body>
        ...
        \\  const s=f.join(' ');
        \\  cmd.textContent=s;
        \\  navigator.clipboard.writeText(s).then(()=>st.textContent='Copied to clipboard.\n'+st.textContent).catch(()=>{});
        \\}
